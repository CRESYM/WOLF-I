...
include(joinpath(@__DIR__, "..", "src", "linmodfun.jl"))
using .ssmatrix # Import the module ssmatrix.
using .ymatrix # Import the module ymatrix.
using .solvelf # Import the module solvelf.

# Solving the load flow problem.
Sb, result, data = solvelf_pm(joinpath(@__DIR__, "..", "data", "bs_2area4gen.m"))

# Admittance matrix
Y_ex = yexmatrix(data) # Expanded admittance matrix (real/imaginary parts)
