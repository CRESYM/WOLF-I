...
The interconnecting transmission network is represented by the node equation : 
...
$$C_D \Delta x + D_D \Delta v = Y_N \Delta v$$

$$\rightarrow \Delta v = (Y_N - D_D)^{-1} C_D x$$

$$\rightarrow \Delta \dot{x} = A_D x + B_D (Y_N - D_D)^{-1} C_D x = Ax$$
